<form action="<?php echo esc_url(admin_url().'admin.php?page=next-social-sharing&tab=display');?>" name="setting_ebay_form" method="post" >
	...
	<div class="disply-login-div">
		...
		<?php if(is_array($buttonstyle)){
			...
			<div class="<?php echo esc_attr('themeDev-form');?> style-sec-next <?php echo ($value == $k ) ? 'style-active' : ''; ?>">
				<?php if( in_array($k, ['button1', 'button2', 'button3', 'button4', 'button5', 'button6', 'button7', 'button8', 'button9', 'button10', 'button11', 'button12', 'button13', 'button14', 'button15', 'button16']) ):
					$proclass = '';
					?>
				...
				<label class="next-display-label <?php echo esc_attr($proclass);?>" for="themedev-style-<?php echo esc_html($k);?>">
					...
					<figure class="image-figure">
						<img src="<?php echo esc_url( NEXT_SOCIAL_FEED_PLUGIN_URL.'assets/images/sharing/styles/'.esc_html($k).'.png' ); ?>" alt="<?php echo esc_html($k);?>">
					</figure>
				</label>
			</div>
